...
{#if $selectedOcrStore}
	<ScrollArea
		class="scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200 relative h-[90vh] overflow-auto"
	>
		<div class="px-2 py-5">
			{#if isPending($selectedOcrStore.currentStep)}
				<h1 class="animate-pulse text-xl">
					Currently processing {$selectedOcrStore.originalFilename} . Please wait up to 5 minutes. In
					the meantime, feel free to upload additional files or continue using the application.
				</h1>
				<h2 class="py-5">
					Stage:
